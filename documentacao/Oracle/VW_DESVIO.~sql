...
CASE 
  WHEN NVL(SUM(REPLACE(vrc.VL_REALIZADO,'.','')),0) = 0 THEN 0
  ELSE ROUND((NVL(SUM(REPLACE(vrc.VL_ORCADO,'.','')),0) / NVL(SUM(REPLACE(vrc.VL_REALIZADO,'.','')),0)) * 100,2)
END AS PORC_DESVIO,
MAX((SELECT NVL(SUM(REPLACE(VL_ORCADO,'.','')),0) 
     FROM orcamento_contabil.VW_RESULTADOS_CONSOLIDADOS
     WHERE SUBSTR(PERIODO,0,2) <= SUBSTR(vrc.PERIODO,0,2) 
     AND SUBSTR(PERIODO,4,4) = SUBSTR(vrc.PERIODO,4,4))) AS VL_ORCADO_ACUMULADO,
MAX((SELECT NVL(SUM(REPLACE(VL_REALIZADO,'.','')),0) 
     FROM orcamento_contabil.VW_RESULTADOS_CONSOLIDADOS
     WHERE SUBSTR(PERIODO,0,2) <= SUBSTR(vrc.PERIODO,0,2) 
     AND SUBSTR(PERIODO,4,4) = SUBSTR(vrc.PERIODO,4,4))) AS VL_ORCADO_ACUMULADO,
CASE 
  WHEN MAX((SELECT NVL(SUM(REPLACE(VL_REALIZADO,'.','')),0) 
     FROM orcamento_contabil.VW_RESULTADOS_CONSOLIDADOS
     WHERE SUBSTR(PERIODO,0,2) <= SUBSTR(vrc.PERIODO,0,2) 
     AND SUBSTR(PERIODO,4,4) = SUBSTR(vrc.PERIODO,4,4))) = 0 THEN 0
  ELSE ROUND((MAX((SELECT NVL(SUM(REPLACE(VL_ORCADO,'.','')),0) 
     FROM orcamento_contabil.VW_RESULTADOS_CONSOLIDADOS
     WHERE SUBSTR(PERIODO,0,2) <= SUBSTR(vrc.PERIODO,0,2) 
     AND SUBSTR(PERIODO,4,4) = SUBSTR(vrc.PERIODO,4,4))) /
MAX((SELECT NVL(SUM(REPLACE(VL_REALIZADO,'.','')),0) 
     FROM orcamento_contabil.VW_RESULTADOS_CONSOLIDADOS
     WHERE SUBSTR(PERIODO,0,2) <= SUBSTR(vrc.PERIODO,0,2) 
     AND SUBSTR(PERIODO,4,4) = SUBSTR(vrc.PERIODO,4,4)))  * 100),2)
END AS PORC_DESVIO_ACUMULADO
--SELECT *
FROM orcamento_contabil.VW_RESULTADOS_CONSOLIDADOS vrc
WHERE vrc.CLASSIFICACAO_CONTABIL = 'RECEITA'
