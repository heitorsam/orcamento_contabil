<?php
	...
	if((isset($_POST['login'])) && (isset($_POST['senha']))){
		...
		$result_usuario = oci_parse($conn_ora, "SELECT orcamento_contabil.VALIDA_SENHA_FUNC_CONTABIL(:usuario,:senha) AS RESP_LOGIN,
												(SELECT INITCAP(usu.NM_USUARIO)
													...
													CASE
														WHEN :usuario IN (SELECT DISTINCT puia.CD_USUARIO
																			...
																			WHERE puia.CD_PAPEL = 367) THEN 'S' --ORÇAMENTO CONTABIL CONTABILIDADE
														ELSE 'N'
													END SN_CONTABILIDADE,
													CASE
														WHEN :usuario IN (SELECT DISTINCT puia.CD_USUARIO
																			FROM dbasgu.PAPEL_USUARIOS puia
																			WHERE puia.CD_PAPEL = 368) THEN 'S' --ORÇAMENTO CONTABIL VISUALIZAÇÃO
														ELSE 'N'
													END SN_VISUALIZACAO
		...
		if(isset($resultado)){
			...
			if($resultado[0] == 'Login efetuado com sucesso') {
				...
				$_SESSION['SN_CONTABILIDADE'] = $resultado[2];
				$_SESSION['SN_VISUALIZACAO'] = $resultado[3];
